...
#define HW_NAME     "snic"
#define VENDOR_ID   0x1137
#define DEVICE_ID   0x0046
...
static void pci_hw_realize(PCIDevice *pci_dev, Error **errp) {
    ...
    memory_region_init_io(&d->mmio2, OBJECT(d), &hw_mmio_ops, d,
                          "hw-mmio2", 0x4000);
    pci_register_bar(pci_dev, 1, PCI_BASE_ADDRESS_SPACE_MEMORY, &d->mmio2);
    memory_region_init_io(&d->io, OBJECT(d), &hw_mmio_ops, d,
                          "hw-io", 0x4000);
    pci_register_bar(pci_dev, 2, PCI_BASE_ADDRESS_SPACE_MEMORY, &d->io);
